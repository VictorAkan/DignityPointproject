import React from "react";

import { useNavigate } from "react-router-dom";

import { Button, Img, Input, Text } from "components";
import Header2 from "components/Header2";

const StoriesCreateStoryOnePage = () => {
  const navigate = useNavigate();

  return (
    <>
      <div className="bg-white-A700_01 flex flex-col font-poppins items-center justify-start mx-auto w-full">
        <div className="flex flex-col gap-[9px] items-center justify-start w-full">
          <Header2 className="bg-white-A700 flex gap-2.5 items-center justify-center md:px-5 px-[50px] py-2.5 shadow-bs w-full" />
          <div className="flex md:flex-col flex-row gap-3 items-center justify-start max-w-[1412px] mx-auto md:px-5 w-full">
            <div className="bg-white-A700 flex md:flex-1 flex-col gap-[22px] items-center justify-start p-2.5 w-[23%] md:w-full">
              <div className="flex flex-col gap-2.5 items-start justify-start mt-[7px] w-auto">
                <div className="flex flex-row gap-2.5 items-center justify-between w-[300px]">
                  <Text
                    className="text-black-900_01 text-center text-xl w-auto"
                    size="txtPoppinsSemiBold20Black90001"
                  >
                    Create story
                  </Text>
                  <Img
                    className="h-[18px] w-[18px]"
                    src="images/img_search.svg"
                    alt="search_One"
                  />
                </div>
                <div className="flex flex-row gap-2.5 items-center justify-start w-auto">
                  <div className="border-b-2 border-gray-500_01 border-solid flex flex-row gap-[5px] items-center justify-start px-[5px] py-2.5 w-[145px]">
                    <Img
                      className="h-[18px] w-[18px]"
                      src="images/img_vector_black_900_01_18x18.svg"
                      alt="vector_One"
                    />
                    <Text
                      className="text-base text-gray-700 w-auto"
                      size="txtPoppinsSemiBold16Gray700"
                    >
                      Photo
                    </Text>
                  </div>
                  <Button
                    className="common-pointer bg-transparent cursor-pointer flex items-center justify-center min-w-[145px]"
                    onClick={() => navigate("/pagepagesyoufollowone")}
                    leftIcon={
                      <div className="mt-0.5 mb-[3px] mr-[5px] bg-black-900_01">
                        <Img
                          src="images/img_vector_black_900_01_18x22.svg"
                          alt="Vector"
                        />
                      </div>
                    }
                    size="md"
                  >
                    <div className="font-semibold leading-[normal] text-base text-gray-700 text-left">
                      Text
                    </div>
                  </Button>
                </div>
              </div>
              <div className="flex flex-col gap-5 items-start justify-start mb-[77px] w-auto">
                <div className="flex flex-col gap-[5px] items-start justify-start w-[300px]">
                  <Text
                    className="text-base text-black-900_01 w-auto"
                    size="txtPoppinsSemiBold16Black90001"
                  >
                    Posting to my stories
                  </Text>
                  <div className="flex flex-row gap-[5px] items-center justify-start w-[300px]">
                    <Img
                      className="h-[50px] md:h-auto rounded-[50%] w-[50px]"
                      src="images/img_jonas.png"
                      alt="jonas_One"
                    />
                    <div className="flex flex-col items-start justify-start w-auto">
                      <Text
                        className="text-base text-black-900_01 text-center w-auto"
                        size="txtPoppinsSemiBold16Black90001"
                      >
                        Endurance Alex
                      </Text>
                      <Text
                        className="text-[13px] text-gray-700 w-auto"
                        size="txtPoppinsMedium13"
                      >
                        @alexander_jnr
                      </Text>
                    </div>
                  </div>
                </div>
                <div
                  className="common-pointer flex flex-row gap-2.5 items-center justify-start w-auto"
                  onClick={() => navigate("/homecreatenewposttagpeople")}
                >
                  <div className="flex relative w-[90px]">
                    <div className="flex my-auto w-[78%]">
                      <div className="flex my-auto w-[72%]">
                        <Img
                          className="h-[30px] my-auto rounded-[50%] w-[30px]"
                          src="images/img_tracy_30x30.png"
                          alt="tracy"
                        />
                        <Img
                          className="h-[30px] ml-[-10px] my-auto rounded-[50%] w-[30px] z-[1]"
                          src="images/img_ruby_30x30.png"
                          alt="ruby"
                        />
                      </div>
                      <Img
                        className="h-[30px] ml-[-10px] my-auto rounded-[50%] w-[30px] z-[1]"
                        src="images/img_mark_30x30.png"
                        alt="mark"
                      />
                    </div>
                    <Img
                      className="h-[30px] ml-[-10px] my-auto rounded-[50%] w-[30px] z-[1]"
                      src="images/img_betty_30x30.png"
                      alt="betty"
                    />
                  </div>
                  <Text
                    className="text-base text-light_blue-900 underline w-auto"
                    size="txtPoppinsSemiBold16Lightblue900"
                  >
                    Tag people
                  </Text>
                </div>
                <Input
                  name="choosepackage"
                  placeholder="Add a caption(optional)"
                  className="font-medium leading-[normal] p-0 placeholder:text-gray-500_01 text-[13px] text-left w-full"
                  wrapClassName="w-full"
                  shape="round"
                  color="indigo_50"
                  size="xl"
                  variant="fill"
                ></Input>
                <div className="bg-indigo-50 flex flex-col gap-[5px] items-center justify-center py-[50px] rounded-lg w-[300px]">
                  <Button
                    className="flex h-[52px] items-center justify-center rounded-[50%] w-[52px]"
                    shape="circle"
                    color="white_A700"
                    size="md"
                    variant="fill"
                  >
                    <Img
                      className="h-8"
                      src="images/img_vector.svg"
                      alt="vector_Three"
                    />
                  </Button>
                  <Text
                    className="text-base text-black-900_01 w-auto"
                    size="txtPoppinsSemiBold16Black90001"
                  >
                    Add Photos/Videos
                  </Text>
                  <Text
                    className="text-[13px] text-gray-700 w-auto"
                    size="txtPoppinsMedium13"
                  >
                    or drag and drop files here
                  </Text>
                </div>
                <div className="bg-indigo-50 flex flex-col items-center justify-start p-[5px] rounded-lg w-full">
                  <div className="flex flex-row items-center justify-between my-1.5 w-[280px]">
                    <Text
                      className="text-[13px] text-gray-700 w-auto"
                      size="txtPoppinsMedium13"
                    >
                      Add something more
                    </Text>
                    <Img
                      className="h-6 w-[89px]"
                      src="images/img_upload.svg"
                      alt="upload"
                    />
                  </div>
                </div>
                <Button
                  className="common-pointer cursor-pointer font-semibold leading-[normal] rounded-[12px] text-base text-center w-[300px]"
                  onClick={() => navigate("/loading")}
                  color="light_blue_900"
                  size="md"
                  variant="fill"
                >
                  Post
                </Button>
              </div>
            </div>
            <div className="bg-white-A700 flex md:flex-1 sm:flex-col flex-row gap-[18px] items-start justify-start md:px-10 sm:px-5 px-[242px] rounded-[20px] w-[77%] md:w-full">
              <Text
                className="sm:mt-0 mt-4 text-black-900_01 text-center text-xl"
                size="txtPoppinsSemiBold20Black90001"
              >
                Preview
              </Text>
              <div className="bg-gray-700 flex flex-col items-start justify-start py-2.5 w-[68%] sm:w-full">
                <div className="flex flex-col gap-[19px] items-center justify-start ml-2.5 md:ml-[0] w-[95%] md:w-full">
                  <Img
                    className="h-1 w-[380px]"
                    src="images/img_frame9057.svg"
                    alt="frame9057"
                  />
                  <div className="flex flex-row items-center justify-between w-[380px] sm:w-full">
                    <div className="flex flex-row gap-2.5 items-center justify-start w-auto">
                      <Img
                        className="h-[50px] md:h-auto rounded-[50%] w-[50px]"
                        src="images/img_jonas.png"
                        alt="jonas_Two"
                      />
                      <div className="flex flex-col gap-0.5 items-start justify-center w-auto">
                        <Text
                          className="text-base text-center text-white-A700 w-auto"
                          size="txtPoppinsSemiBold16WhiteA700"
                        >
                          My Story
                        </Text>
                        <div className="flex flex-row gap-5 items-center justify-start w-auto">
                          <Text
                            className="text-[13px] text-white-A700 w-auto"
                            size="txtPoppinsMedium13WhiteA700"
                          >
                            @alexander_jnr
                          </Text>
                          <div className="flex flex-row gap-[5px] items-center justify-start w-auto">
                            <Img
                              className="h-[18px] w-[18px]"
                              src="images/img_whatsappstatus.svg"
                              alt="whatsappstatus"
                            />
                            <Text
                              className="text-[13px] text-white-A700"
                              size="txtPoppinsMedium13WhiteA700"
                            >
                              1hr
                            </Text>
                          </div>
                        </div>
                      </div>
                    </div>
                    <Img
                      className="h-[15px] w-3.5"
                      src="images/img_thumbsup_white_a700.svg"
                      alt="thumbsup"
                    />
                  </div>
                </div>
                <Img
                  className="h-[558px] sm:h-auto mt-[18px] object-cover w-full"
                  src="images/img_image91.png"
                  alt="imageNinetyOne"
                />
                <Text
                  className="mb-[51px] ml-2.5 md:ml-[0] mt-3.5 text-base text-white-A700"
                  size="txtPoppinsSemiBold16WhiteA700"
                >
                  Caption goes here
                </Text>
              </div>
            </div>
          </div>
        </div>
      </div>
    </>
  );
};

export default StoriesCreateStoryOnePage;
